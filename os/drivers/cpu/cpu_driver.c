...
static int cpu_ioctl(FAR struct file *filep, int cmd, unsigned long arg)
{
    ...
    switch (cmd) {
    case IOC_CPU_ENABLE:
		/* Driver will call hotplug_manager's cpu_online() API to Power on the CPU
		* and schedule the tasks.
		*/
        return cpu_online(dev->cpu);
    case IOC_CPU_DISABLE:
		/* Driver will call hotplug managers cpu_offline() API to migrate the
		* secheduled tasks and power down the CPU.
		*/
		return cpu_offline(dev->cpu);
